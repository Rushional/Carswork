...
public class QueriesData {
    public static QueryData workers() {
        return new QueryData(
                new String[]{"ФИО", "Номер телефона", "Дата рождения", "Дата принятия на работу5"},
                "SELECT worker_name, phone_number, birthday, hire_date\n" +
                        "FROM worker"
        );
    }
    ...
    public static QueryData ownedModels() {
        return new QueryData(
                new String[]{"Название", "Отзывы", "Стоимость аренды"},
                "SELECT model_name, user_feedback, rent_cost\n" +
                        "FROM car_model\n" +
